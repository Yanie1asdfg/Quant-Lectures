...
class Varieties_Contracts_pre():
    ...
    # 从东方财富获取单个品种合约数据
    def spider_data(self,exchange,num):
        '''
        exchange:交易所代码
        num:合约代码
        '''
    ...
    def spider_datas(self):
        ...
        for e in exchange:
            for i in range(30):
                code,name,data = self.spider_data(e,i)
                # print(code,name)
                if code!=0:
    ...
    #                 e = e+1
        for y in range(1, IMAGE_ROW + 1):
            ...
            while e<len(images_size) and x_pos+images_size[e]<IMAGE_c_SIZE-100:
                print(imagelist[e],x_pos,images_size[e],int(images_size[e]/100-6)*20+380)
...
if __name__ == '__main__':
    vc = Varieties_Contracts_pre()
    # vc.spider_datas()
    # print(vc.spider_data)
    dbname = 'db_eastmoney'
    # vc.contlist_from_mysql(dbname)
    dict1 = {'白糖':'SR','豆粕':'M','玉米':'C','玉米淀粉':'CS','苹果':'AP','菜粕':'RM','纯碱':'SA','玻璃':'FG','甲醇':'MA','棉花':'CF','尿素':'UR'} 
    vc.get_all_data(dbname,dict1)
    vc.all_plot()
    date = '2021-07-30'
    vc.syn_images(date)